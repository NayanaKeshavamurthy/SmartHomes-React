...
app.post('/api/addOrder', async (req, res) => {
  ...
  try {
      await connection.promise().query('INSERT INTO orders (orderID,customerName,address,city,state,zip,creditCardNumber,deilveryOption,phoneNumber,order_Date,shippping_date,total,storeId) VALUES (?,?, ?, ?,?,?,?,?,?,?,?,?,?)', [od,username, address, city,state,zip,cc,dlop,phno,order_date,shippingdate,total,storeid]);
      res.json({ success: true })
  } catch (error) {
